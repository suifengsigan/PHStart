...
Module CampbellCsvToAfu
    ...
    Public freqUnits As New Hashtable
    Public whirlDir As New Hashtable
...
#End Region
    ...
    Public Sub initialize_cb()
        Try
            ...
            Dim propList As PropertyList = theDialog.TopBlock.GetProperties
            propList.SetString("Label", "NXN Rotordynamics Plots")
            propList.Dispose()

            propList = group0.GetProperties
            propList.SetString("Label", "Input File")
            propList.Dispose()

            propList = nativeFileBrowser0.GetProperties
            propList.SetString("Label", "Nastran ROTCSV file")
            propList.SetString("Filter", "*.csv")
            propList.Dispose()

            propList = group.GetProperties
            propList.SetString("Label", "Axis Units")
            propList.Dispose()

            Dim unitList(freqUnits.Count - 1) As String
            freqUnits.Keys.CopyTo(unitList, 0)

            propList = enum0.GetProperties
            propList.SetString("Label", "Rotor Speed (RUNIT)")
            propList.SetEnumMembers("Value", unitList)
            propList.Dispose()

            propList = enum01.GetProperties
            propList.SetString("Label", "Freq. Output (FUNIT)")
            propList.SetEnumMembers("Value", unitList)
            propList.Dispose()

            propList = group1.GetProperties()
            propList.SetString("Label", "Processing Options")
            propList.Dispose()

            propList = label0.GetProperties()
            propList.SetString("Label", "Analysis System (REFSYS)")
            propList.Dispose()

            propList = toggle0.GetProperties()
            propList.SetString("Label", "Eigenfrequencies")
            propList.Dispose()

            propList = toggle01.GetProperties()
            propList.SetString("Label", "Damping Values")
            propList.Dispose()

            propList = toggle02.GetProperties()
            propList.SetString("Label", "Real part of eigensolution")
            propList.Dispose()

            propList = toggle03.GetProperties()
            propList.SetString("Label", "Imaginary part of eigensolution*")
            propList.Dispose()

            propList = toggle04.GetProperties()
            propList.SetString("Label", "Whirl Direction")
            propList.Dispose()

            propList = label01.GetProperties()
            propList.SetString("Label", "Converted System")
            propList.Dispose()

            propList = toggle05.GetProperties()
            propList.SetString("Label", "Eigenfrequencies")
            propList.Dispose()

            propList = toggle06.GetProperties()
            propList.SetString("Label", "Whirl Direction")
            propList.Dispose()

            propList = toggle07.GetProperties()
            propList.SetString("Label", "Imaginary part of eigensolution*")
            propList.Dispose()

            propList = toggle08.GetProperties()
            propList.SetString("Label", "Real part of eigensolution*")
            propList.Dispose()

            propList = label02.GetProperties()
            propList.SetString("Label", "                    * MODTRK=2/3 only")
            propList.Dispose()

            propList = toggle09.GetProperties()
            propList.SetString("Label", "Per Rev Lines (1P, 2P, etc.)")
            propList.Dispose()

            propList = group2.GetProperties
            propList.SetString("Label", "Output File")
            propList.SetLogical("Show", False)
            propList.Dispose()

            propList = nativeFileBrowser01.GetProperties
            propList.SetString("Label", "NX AFU file")
            propList.SetString("Filter", "*.afu")
            propList.Dispose()
    ...
    Public Function apply_cb() As Integer
        ...
        Try
            ...
            '---- Enter your callback code here -----
            Dim propList As PropertyList = nativeFileBrowser0.GetProperties
            csvFile = propList.GetString("Path")
            propList.Dispose()

            propList = enum0.GetProperties
            rUnit = propList.GetEnumAsString("Value")
            propList.Dispose()

            propList = enum01.GetProperties
            fUnit = propList.GetEnumAsString("Value")
            propList.Dispose()

            propList = toggle0.GetProperties()
            options(0) = propList.GetLogical("Value")
            propList.Dispose()

            propList = toggle01.GetProperties()
            options(1) = propList.GetLogical("Value")
            propList.Dispose()

            propList = toggle02.GetProperties()
            options(2) = propList.GetLogical("Value")
            propList.Dispose()

            propList = toggle03.GetProperties()
            options(3) = propList.GetLogical("Value")
            propList.Dispose()

            propList = toggle04.GetProperties()
            options(4) = propList.GetLogical("Value")
            propList.Dispose()

            propList = toggle05.GetProperties()
            options(5) = propList.GetLogical("Value")
            propList.Dispose()

            propList = toggle06.GetProperties()
            options(6) = propList.GetLogical("Value")
            propList.Dispose()

            propList = toggle07.GetProperties()
            options(7) = propList.GetLogical("Value")
            propList.Dispose()

            propList = toggle08.GetProperties()
            options(8) = propList.GetLogical("Value")
            propList.Dispose()

            propList = toggle09.GetProperties()
            options(9) = propList.GetLogical("Value")
            propList.Dispose()

            propList = nativeFileBrowser01.GetProperties
            afuFile = propList.GetString("Path")
            propList.Dispose()
    ...
    Public Function update_cb(ByVal block As NXOpen.BlockStyler.UIBlock) As Integer
        Try
            ...
            If block Is nativeFileBrowser0 Then
                ...
                ''Dim propList As PropertyList = nativeFileBrowser0.GetProperties
                ''Dim csvFile As String = propList.GetString("Path")
                ''propList.Dispose()

                ''Dim afuFile As String = IO.Path.ChangeExtension(csvFile, ".afu")
                ''propList = nativeFileBrowser01.GetProperties
                ''propList.SetString("Path", afuFile)
                ''propList.Dispose()
